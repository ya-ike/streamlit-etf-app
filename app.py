...
from signal_engine_v6 import (
    ...
    download_price_data, run_signal_for_date, run_signal_for_execution_date, apply_same_day_prices
)

JST = ZoneInfo("Asia/Tokyo")
APP_VERSION = "v2026-05-19-shared-06"
...
def load_saved_state():
    ...
    if signal_date and not daily_all.empty:
        if "使用日本日付" in daily_all.columns:
            daily_df = daily_all[daily_all["使用日本日付"].astype(str).apply(normalize_date_like_text) == signal_date].copy()
        elif "シグナル日付" in daily_all.columns:
...
st.caption(f"計算は signal_engine_v4.py を使用。朝一計算、午後は当日の午前寄り値 / 午後引け値を記入します。 / {APP_VERSION}")
    ...
    if locked:
        ...
        try:
            with st.spinner("朝の確定計算を実行中..."):
                ...
                execution_date = pd.Timestamp(now_jst().strftime("%Y-%m-%d"))
                signal_df, daily_df, trade_df = run_signal_for_execution_date(execution_date, close_df, open_df, volume_df, settings)

                execution_text = execution_date.strftime("%Y-%m-%d")
                if "シグナル日付" in signal_df.columns:
                    signal_df = signal_df.drop(columns=["シグナル日付"])
                if "シグナル日付" in daily_df.columns:
                    daily_df = daily_df.drop(columns=["シグナル日付"])
                signal_date_text = execution_text
    ...
    with c1:
        if st.button("当日の午前寄り値 / 午後引け値を反映", use_container_width=True):
            try:
                ...
                signal_date = pd.Timestamp(str(daily_df.iloc[0]["使用日本日付"]))
