...
APP_VERSION = "v2026-05-19-shared-05"
    ...
    if locked:
        ...
        try:
            with st.spinner("朝の確定計算を実行中..."):
                ...
                execution_text = now_jst().strftime("%Y-%m-%d")
                # Streamlit運用では日本の実行日を当日で固定
                if "使用日本日付" in signal_df.columns:
                    signal_df["使用日本日付"] = execution_text
                if "使用日本日付" in daily_df.columns:
                    daily_df["使用日本日付"] = execution_text
                # シグナル日付欄は使わない
