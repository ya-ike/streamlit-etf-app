...
def build_signal_log_df(score_df: pd.DataFrame, aligned_jp_index, aligned_us_index, settings: dict, used_us_date: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    if used_us_date is not None and not pd.isna(used_us_date):
        latest_us_date = pd.Timestamp(used_us_date).strftime("%Y-%m-%d")
...
def run_signal_for_date(signal_date: pd.Timestamp, close_df: pd.DataFrame, open_df: pd.DataFrame, volume_df: pd.DataFrame, settings: dict):
    ...
    prev_us = us_ret.index[us_ret.index < signal_date]
    used_us_date = prev_us[-1] if len(prev_us) > 0 else pd.NaT
    ...
    signal_df = build_signal_log_df(score_df, aligned_jp.index, aligned_us.index, settings, used_us_date=used_us_date)
...
def run_signal_for_execution_date(execution_date: pd.Timestamp, close_df: pd.DataFrame, open_df: pd.DataFrame, volume_df: pd.DataFrame, settings: dict):
    execution_date = pd.Timestamp(execution_date).normalize()
    us_ret = calc_us_close_to_close_returns(close_df)
    jp_ret = calc_jp_open_to_close_returns(open_df, close_df)

    aligned_us, aligned_jp = align_us_to_jp(us_ret, jp_ret)
    # 実行日の日本バーはまだ未確定なので、学習は実行日より前まで
    aligned_us = aligned_us[aligned_us.index < execution_date]
    aligned_jp = aligned_jp[aligned_jp.index < execution_date]

    if aligned_us.empty or aligned_jp.empty:
        raise RuntimeError("日米営業日の対応付けに失敗しました。")

    used_us_candidates = us_ret.index[us_ret.index < execution_date]
    if len(used_us_candidates) == 0:
        raise RuntimeError("使用米国日付が取得できません。")
    used_us_date = used_us_candidates[-1]
    ...
    latest_us_row = us_ret.loc[used_us_date]
    latest_x_z = ((latest_us_row - us_mean) / us_std).replace([np.inf, -np.inf], np.nan).fillna(0.0).values.reshape(1, -1)
    latest_factor = latest_x_z @ v
    ...
    score_df = calculate_suggested_quantity(score_df, close_df.loc[:execution_date], volume_df.loc[:execution_date], settings)
    signal_df = build_signal_log_df(score_df, aligned_jp.index, aligned_us.index, settings, used_us_date=used_us_date)
    # 表示上は日本の実行日を当日にする
    signal_df["使用日本日付"] = pd.Timestamp(execution_date).strftime("%Y-%m-%d")
    daily_df = build_daily_summary_df(signal_df)
    trade_df = build_trade_input_df(signal_df)
    trade_df["売買日"] = pd.Timestamp(execution_date).strftime("%Y-%m-%d")
    return signal_df, daily_df, trade_df
